...
def install_dependencies(dev=False):
    ...
    _run_command("pipenv install --three --deploy {}".format("--dev" if dev else ""))
...
def docs(dummy_build=False):
    ...
    _run_command(
        "pipenv run sphinx-build -nWT -b {} doc doc/_build/html".format(builder)
    )
...
def test_docker():
    ...
    for img in images:
        ...
        volumes = {SCRIPT_DIR: {"bind": "/test", "mode": "ro"}}
...
def _run_command(command, pipe_output=True):
    ...
    process = subprocess.Popen(
        command.split(), stdout=stdout, stderr=stdout, universal_newlines=True
    )
    if pipe_output:
...
if __name__ == "__main__":
    ...
    group.add_argument(
        "-d",
        "--detached",
        help="Start TradingBot as a background process",
        action="store_true",
    )
    group.add_argument(
        "-s", "--stop", help="Stop TradingBot background process", action="store_true"
    )
    group.add_argument(
        "-c",
        "--close-positions",
        help="Start TradingBot to close all open positions",
        action="store_true",
    )
    group.add_argument(
        "-i", "--install", help="Install TradingBot", action="store_true"
    )
    group.add_argument(
        "--test", help="Start TradingBot automatic test suite", action="store_true"
    )
    group.add_argument(
        "--docs", help="Build TradingBot html documentation", action="store_true"
    )
    group.add_argument(
        "--prepare", help="Install TradingBot dependencies", action="store_true"
    )
    group.add_argument(
        "--prepare-dev",
        help="Install TradingBot development dependencies",
        action="store_true",
    )
    group.add_argument(
        "--build-docker", help="Build TradingBot Docker image", action="store_true"
    )
    group.add_argument(
        "--start-docker",
        help="Start TradingBot in a detached docker container",
        action="store_true",
    )
    group.add_argument(
        "--test-docker",
        help="Start TradingBot automatic test suite in docker containers",
        action="store_true",
    )
    group.add_argument(
        "--stop-docker", help="Stop TradingBot docker container", action="store_true"
    )
